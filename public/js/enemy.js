...
(function () {
    ...
    function getEnemyCoreWorldPosition(enemy, outVec3) {
        if (!enemy) return null;
        var out = outVec3 || new THREE.Vector3();
        if (enemy.rigApi && enemy.rigApi.getSocketWorldPosition) {
            var fromSocket = enemy.rigApi.getSocketWorldPosition('core_anchor', out);
            if (fromSocket) return fromSocket;
        }
        if (enemy.rigApi && enemy.rigApi.getCoreWorldPosition) {
            return enemy.rigApi.getCoreWorldPosition(out);
        }
    ...
    function getEnemyMuzzleWorldPosition(enemy, outVec3) {
        if (!enemy) return null;
        var out = outVec3 || new THREE.Vector3();
        if (enemy.rigApi && enemy.rigApi.getSocketWorldPosition) {
            var fromSocket = enemy.rigApi.getSocketWorldPosition('muzzle_socket', out);
            if (fromSocket) return fromSocket;
        }
        if (enemy.rigApi && enemy.rigApi.getMuzzleWorldPosition) {
            return enemy.rigApi.getMuzzleWorldPosition(out);
        }
        if (enemy.weaponMuzzle && enemy.weaponMuzzle.getWorldPosition) {
            enemy.weaponMuzzle.getWorldPosition(out);
            return out;
        }
        out.copy(enemy.group.position);
    ...
    function updateEnemyAnimation(enemy, dt, engaging) {
        ...
        var aimPitch = engaging ? -0.05 : 0;
        var sprinting = speedNorm > 0.85;

        if (enemy.rigApi && enemy.rigApi.updatePose) {
            enemy.rigApi.updatePose({
                moveSpeedNorm: speedNorm,
                sprinting: sprinting,
                aimPitch: aimPitch,
                equippedWeaponId: enemy.weaponType
            }, dt);
            return;
        }
        if (enemy.rigApi && enemy.rigApi.updateLocomotion) {
            enemy.rigApi.updateAimPitch(aimPitch);
            enemy.rigApi.updateLocomotion(speedNorm, sprinting, dt);
            return;
        }
        if (!enemy.rig) return;
    ...
    function updateCombat(enemy, dt, playerPos, onPlayerHit) {
        ...
        var muzzlePos = getEnemyMuzzleWorldPosition(enemy, enemyShootOrigin);
        if (!muzzlePos) return false;
        enemyShootOrigin.copy(muzzlePos);

        enemyShootTarget.copy(playerPos);
        enemyShootTarget.y += 0.4;
        ...
        if (enemy.rigApi && enemy.rigApi.setMuzzleVisible) {
            enemy.rigApi.setMuzzleVisible(true);
            enemy.muzzleFlashTimer = 0.06;
        } else if (enemy.weaponMuzzle) {
    ...
    function updateMuzzleFlash(enemy, dt) {
        if (enemy.muzzleFlashTimer <= 0) return;

        enemy.muzzleFlashTimer -= dt;
        if (enemy.muzzleFlashTimer <= 0) {
            if (enemy.rigApi && enemy.rigApi.setMuzzleVisible) {
                enemy.rigApi.setMuzzleVisible(false);
            } else if (enemy.weaponMuzzle) {
    ...
    GameEnemy.getLockTargets = function () {
        ...
        for (var i = 0; i < enemies.length; i++) {
            ...
            var corePos = getEnemyCoreWorldPosition(enemy, new THREE.Vector3());
            if (!corePos) continue;
