...
(function () {
    ...
    var lastPlasmaSfxActive = false;
    var lastPlasmaSfxOverheated = false;
    ...
    function applyWeapon(weapon) {
        ...
        if (window.GameAudio && window.GameAudio.playWeaponSwitch) {
            window.GameAudio.playWeaponSwitch();
        }
    ...
    function setupPerspectiveControls() {
        ...
        document.addEventListener('keydown', function (e) {
            ...
            if (e.code === 'KeyX' && window.GamePlayer && window.GamePlayer.swapShoulder) {
                var side = window.GamePlayer.swapShoulder();
                setTransientDebug('Shoulder: ' + String(side || 'right').toUpperCase(), 900);
            }
        });
    ...
    function handleEnemyHit(hitPoint, damage, hitType, result) {
        ...
        if (result.killed) {
            ...
            if (window.GameAudio && window.GameAudio.playKill) {
                window.GameAudio.playKill();
            }
        } else {
        ...
        if (window.GameAudio && window.GameAudio.playHit) {
            window.GameAudio.playHit(hitType);
        }
    }
    ...
    function consumePlayerDamage(rawDamage, hitType, attackerEnemy) {
        ...
        if (damage > 0) {
            playerHP -= damage;
            if (window.GameAudio && window.GameAudio.playPlayerDamage) {
                window.GameAudio.playPlayerDamage(hitType, rawDamage);
            }
        }
        ...
        if (playerHP <= 0) {
            if (window.GameAudio && window.GameAudio.playPlayerDeath) {
                window.GameAudio.playPlayerDeath();
            }
            respawnPlayer();
            return;
        }
    ...
    function respawnPlayer() {
        ...
        window.GameUI.updateClassInfo(window.GameClasses.getHudState());
        syncWallhackRingRadius();
        if (window.GameAudio && window.GameAudio.playRespawn) {
            window.GameAudio.playRespawn();
        }
    }
    ...
    function tryPlayerFire() {
        ...
        if (fired) {
            ...
            if (window.GameAudio && window.GameAudio.playWeapon) {
                var activeWeapon = window.GameHitscan.getCurrentWeapon ? window.GameHitscan.getCurrentWeapon() : null;
                window.GameAudio.playWeapon(activeWeapon ? activeWeapon.id : 'rifle');
            }
        }
    }
    ...
    function setupShooting() {
        document.addEventListener('mousedown', function (e) {
            ...
            triggerHeld = true;
            if (multiplayerMode && window.GameNet && window.GameNet.sendBeamIntent) {
                var activeWeapon = window.GameHitscan.getCurrentWeapon ? window.GameHitscan.getCurrentWeapon() : null;
                if (activeWeapon && activeWeapon.id === 'plasma') {
                    window.GameNet.sendBeamIntent(true, 'plasma');
                }
            }
            tryPlayerFire();
        });
        ...
        document.addEventListener('mouseup', function (e) {
            ...
            triggerHeld = false;
            if (multiplayerMode && window.GameNet && window.GameNet.sendBeamIntent) {
                window.GameNet.sendBeamIntent(false, 'plasma');
            }
        });
    ...
    function tryThrow(type) {
        ...
        if (outcome.ok && window.GameAudio && window.GameAudio.playThrow) {
            window.GameAudio.playThrow(type);
        }
    ...
    function initGame() {
        ...
        if (window.GameAudio && window.GameAudio.init) {
            ...
            if (window.GameAudio.setGameplayActive) {
                window.GameAudio.setGameplayActive(false);
            }
        }
    ...
    function animate() {
        ...
        var plasmaSfxActive = !!(plasmaState && plasmaState.active && hasInputCapture() && isPlaying);
        var plasmaSfxOverheated = !!(plasmaState && plasmaState.overheated);
        if (plasmaSfxActive !== lastPlasmaSfxActive || plasmaSfxOverheated !== lastPlasmaSfxOverheated) {
            if (window.GameAudio && window.GameAudio.setPlasma) {
                window.GameAudio.setPlasma(plasmaSfxActive, plasmaSfxOverheated);
            }
            lastPlasmaSfxActive = plasmaSfxActive;
            lastPlasmaSfxOverheated = plasmaSfxOverheated;
        }
        if (plasmaBeamLine) {
