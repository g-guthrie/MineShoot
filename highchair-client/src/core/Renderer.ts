...
// Cap the effective pixel ratio (devicePixelRatio * quality multiplier) at the
// display's native Retina density. Anything above it supersamples pixels the
// screen can't show, and the whole post-processing chain pays that fill-rate
// cost too (on dpr-2 displays the HIGH preset used to render 9x the viewport).
...
export default class Renderer {
  ...
  private _onWindowResize = (): void => {
    ...
    this._renderer.setPixelRatio(this._effectivePixelRatio());
  ...
  private _effectivePixelRatio(): number {
    const { resolution } = this._game.settingsManager.qualityPerfTradeoff;
    return Math.min(window.devicePixelRatio * resolution.multiplier, MAX_EFFECTIVE_PIXEL_RATIO);
  }
