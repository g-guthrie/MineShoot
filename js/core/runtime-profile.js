(function () {
    ...
    function localWorkerOrigin() {
        if (isLocalEnvironment() && isHttpEnvironment()) {
            return String(window.location.origin || LOCAL_WORKER_ORIGIN);
        }
    ...
    function backendOriginFor(kind) {
        ...
        if (kind === 'local-worker') return localWorkerOrigin();
        return '';
    }
