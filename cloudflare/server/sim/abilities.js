...
export function buildDeadeyeState(deadeyeCfg, picks, now) {
  const maxTargets = Math.max(1, Math.round((deadeyeCfg && deadeyeCfg.maxTargets) || 6));
  const durationMs = Math.max(1, Math.round(Number((deadeyeCfg && deadeyeCfg.duration) || 4.0) * 1000));
