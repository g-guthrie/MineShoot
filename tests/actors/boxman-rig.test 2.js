...
test('boxman fully overrides the locked right arm against parent turn motion', () => {
  const parentQuat = new THREE.Quaternion().setFromEuler(new THREE.Euler(0, 0.3, 0, 'XYZ'));
  const neutralParentQuat = new THREE.Quaternion();
  const armUpperQuat = new THREE.Quaternion();
  const armLowerQuat = new THREE.Quaternion();
  const rig = {
    armUpperRParentNeutralWorldQuat: neutralParentQuat.clone(),
    armUpperR: {
      parent: {
        getWorldQuaternion(out) {
          return out.copy(parentQuat);
        }
      },
      quaternion: armUpperQuat,
      rotation: { x: 9, y: 8, z: 7 }
    },
    armLowerR: {
      quaternion: armLowerQuat,
      rotation: { x: 6, y: 5, z: 4 }
    }
  };

  const applied = boxmanRig._test.applyLockedRightArmAimBasePose(rig);
  const upperWorld = parentQuat.clone().multiply(armUpperQuat.clone());
  const neutralWorld = neutralParentQuat.clone().multiply(
    new THREE.Quaternion().setFromEuler(new THREE.Euler(
      ((21.02 - (28 * 1.9)) * (Math.PI / 180)),
      -7.92 * (Math.PI / 180),
      11.86 * (Math.PI / 180),
      'XYZ'
    ))
  ...
  assert.equal(applied, true);
  assert.ok(upperWorld.angleTo(neutralWorld) < 0.000001);
});
...
test('boxman keeps the right-arm aim immune to standing turn clips and idle turn poses while mousing left and right', () => {
  const turnEntryYaw = boxmanRig._test.resolveIdleAimYawState({
    directional: {
      facingYaw: 0,
      useTurnEntryClip: true,
      useTurnLoopClip: false,
      turnClipDirection: -1,
  ...
  const turnLoopYaw = boxmanRig._test.resolveIdleAimYawState({
    directional: {
      facingYaw: 0,
      useTurnEntryClip: false,
      useTurnLoopClip: true,
      turnClipDirection: 1,
      turnLoopPoseWeight: 1,
      idleTurnPoseWeight: 0,
      idleTurnDirection: 0
    }
  }, 0);
  const idleTurnYaw = boxmanRig._test.resolveIdleAimYawState({
    directional: {
      facingYaw: 0,
      useTurnEntryClip: false,
      useTurnLoopClip: false,
      turnClipDirection: 0,
      turnLoopPoseWeight: 0,
      idleTurnPoseWeight: 0.5,
      idleTurnDirection: -1
    }
  }, 0);
  const lockedClipYaw = boxmanRig._test.resolveIdleAimYawState({
    directional: {
      facingYaw: 0,
      useTurnEntryClip: false,
      useTurnLoopClip: false,
  ...
  assert.equal(turnEntryYaw.facingYaw, 0);
  assert.equal(turnLoopYaw.facingYaw, 0);
  assert.equal(idleTurnYaw.facingYaw, 0);
  assert.equal(lockedClipYaw.facingYaw, 0);
});
...
test('boxman uses the same hand mount on the left side for throwable origin', () => {
  const weaponMount = boxmanRig._test.weaponMount();
  const throwableMount = boxmanRig._test.throwableMount();

  assert.deepEqual(throwableMount.rootPos, weaponMount.rootPos);
  assert.deepEqual(throwableMount.rootRot, weaponMount.rootRot);
  assert.deepEqual(throwableMount.originPos, { x: 0, y: 0, z: 0 });
});
