...
(function () {
    ...
    var predictedRemoteHitFeedback = {};
    var PREDICTED_REMOTE_HIT_TTL_MS = 900;
    ...
    function updateMenuSessionPanel(matchState, selfState) {
        ...
        els.kd.textContent = 'KILLS ' + kills + ' | DEATHS ' + deaths;
    ...
    function prunePredictedRemoteHitFeedback(now) {
        var stamp = Number(now || Date.now());
        for (var token in predictedRemoteHitFeedback) {
            if (!Object.prototype.hasOwnProperty.call(predictedRemoteHitFeedback, token)) continue;
            if (Number(predictedRemoteHitFeedback[token].expiresAt || 0) > stamp) continue;
            delete predictedRemoteHitFeedback[token];
        }
    }

    function registerPredictedRemoteHitFeedback(shotToken, hitType, weaponId) {
        var token = String(shotToken || '');
        var stamp = Date.now();
        prunePredictedRemoteHitFeedback(stamp);
        if (token) {
            predictedRemoteHitFeedback[token] = {
                expiresAt: stamp + PREDICTED_REMOTE_HIT_TTL_MS
            };
        }
    ...
    function consumePredictedRemoteHitFeedback(shotToken) {
        var token = String(shotToken || '');
        if (!token || !Object.prototype.hasOwnProperty.call(predictedRemoteHitFeedback, token)) return false;
        delete predictedRemoteHitFeedback[token];
        return true;
    }
    ...
    function validateMenuSelections() {
        ...
        if (!missingWeapons.length) return { ok: true, message: '' };
    ...
    function setupMenuWeaponLoadout() {
        ...
        function assignWeaponToSlot(activeSlot, weaponId) {
            var nextSlots = syncMenuWeaponSlotsToRuntime();
            var selectedId = String(weaponId || '');
            var active = activeSlot === 1 ? 1 : 0;
            var other = active === 0 ? 1 : 0;
            if (!selectedId) return false;
            if (nextSlots[active] === selectedId) return false;
            if (nextSlots[other] === selectedId) {
                nextSlots[other] = '';
            }
            nextSlots[active] = selectedId;
            menuWeaponSlots = nextSlots.slice(0, 2);
            syncMenuWeaponSlotsToRuntime();
            return true;
        }
        ...
        function render() {
            ...
            for (var n = 0; n < available.length; n++) {
                ...
                choice.addEventListener('click', function () {
                    ...
                    if (!assignWeaponToSlot(menuActiveSlot, selectedId)) return;
    ...
    function handleNetworkDamageFeedback(feedback) {
        ...
        var damageNumberSpread = isShotgun ? { spreadX: 152, spreadY: 72 } : undefined;
        var predictedLocally = consumePredictedRemoteHitFeedback(feedback.shotToken || '');

        if ((!predictedLocally || feedback.killed) && globalThis.__MAYHEM_RUNTIME.GameAudio && globalThis.__MAYHEM_RUNTIME.GameAudio.play) {
            globalThis.__MAYHEM_RUNTIME.GameAudio.play('bulletImpact', {
            ...
            function (hitboxMesh, hitPoint, distance, hitType, damage, weapon) {
                if (multiplayerMode && hitboxMesh && hitboxMesh.userData && hitboxMesh.userData.ownerType === 'net') {
                    if (globalThis.__MAYHEM_RUNTIME.GameNet && globalThis.__MAYHEM_RUNTIME.GameNet.sendFire) {
                        ...
                        registerPredictedRemoteHitFeedback(shotToken, hitType, weapon && weapon.id ? weapon.id : 'rifle');
                    }
                    return;
                }
    ...
    function initGame() {
        ...
        function finalizeWorldBootstrap(worldMeta) {
            ...
            globalThis.__MAYHEM_RUNTIME.GamePlayerCombat.applyArmorProfile(globalThis.__MAYHEM_RUNTIME.GamePlayerCombat.getArmorMax());
    ...
    function animate() {
        ...
        if (multiplayerMode) {
            ...
            if (globalThis.__MAYHEM_RUNTIME.GameNet.consumeDamageFeedback) {
                prunePredictedRemoteHitFeedback();
            ...
            updateMenuSessionPanel(null, null);
        }
        ...
        if (globalThis.__MAYHEM_RUNTIME.GamePlayer && globalThis.__MAYHEM_RUNTIME.GamePlayer.setHealFlash) {
            globalThis.__MAYHEM_RUNTIME.GamePlayer.setHealFlash(false);
        }
    ...
    function boot() {
        ...
        if (authApi && authApi.initMenuAuth) {
            authApi.initMenuAuth();
        }
        ...
        function startWithMode(modeId, options) {
            ...
            function proceedStart() {
                if (!startPending || started) return;
                startPending = false;
                ...
                if (selectedMode.authorityMode === 'networked') {
                    var useGuestIdentity = !(authApi && authApi.isLoggedIn && authApi.isLoggedIn());
                    forceGuestNetMode = useGuestIdentity;
                    forcedRoomId = selectedMode.roomId || 'global';
                    if (useGuestIdentity && globalThis.__MAYHEM_RUNTIME.GameNet && globalThis.__MAYHEM_RUNTIME.GameNet.enableGuestMode) {
                        globalThis.__MAYHEM_RUNTIME.GameNet.enableGuestMode();
                    }
                    if (!useGuestIdentity && authApi && authApi.setAuthVisible) {
            ...
            if (selectedMode.authorityMode === 'networked' && authApi && authApi.ensureMenuSession) {
                authApi.ensureMenuSession()
                    .catch(function () {
                        return null;
                    })
                    .finally(proceedStart);
                return;
            }
