...
(function () {
    ...
    function buildWeaponFromShared(id) {
        ...
        return {
            ...
            adsSpread: Number(s.adsSpread != null ? s.adsSpread : s.hipfireSpread || 0),
    ...
    function getWeaponSpreadMetrics(weapon) {
        ...
        var baseSpread = Math.max(0, Number(weapon.hipfireSpread || 0));
        var adsSpread = Math.max(0, Number(weapon.adsSpread != null ? weapon.adsSpread : baseSpread));
        var spread = isAdsActiveForWeapon(weapon.id) ? adsSpread : baseSpread;
        if (spread <= 0.00001) {
            return { radiusPx: 0, radiusXpx: 0, radiusYpx: 0, spread: 0 };
        }
    ...
    function getBloomCircleSizePx(weapon) {
        if (!weapon || weapon.id === 'shotgun' || weapon.singleHitFromPellets) return 0;
        return getWeaponSpreadRadiusPx(weapon) * 2;
    }
    ...
    function getCircleSampleNdcOffset(weapon, sample) {
        ...
        return {
            x: (Number(sample && sample.x || 0) * Number(metrics.radiusXpx || 0)) / (window.innerWidth * 0.5),
            y: (Number(sample && sample.y || 0) * Number(metrics.radiusYpx || 0)) / (window.innerHeight * 0.5)
        };
    }
    ...
    function fireCircleScanPattern(camera, weapon, onHit, onMiss) {
        ...
        if (traced.traceEnd) spawnTracer(camera, weapon.id, traced.traceEnd);
        if (!traced.hit) {
            if (onMiss) onMiss();
            return true;
        }
        ...
        if (globalThis.__MAYHEM_RUNTIME.GamePlayer && globalThis.__MAYHEM_RUNTIME.GamePlayer.getRotation && globalThis.__MAYHEM_RUNTIME.GamePlayer.getPosition) {
            ...
            var playerPos = globalThis.__MAYHEM_RUNTIME.GamePlayer.getPosition();
            playerForward.set(-Math.sin(playerRot.yaw || 0), 0, -Math.cos(playerRot.yaw || 0));
            hitFromPlayer.copy(traced.hitPoint).sub(playerPos).setY(0);
            if (hitFromPlayer.lengthSq() > 0.0001) {
                hitFromPlayer.normalize();
                if (playerForward.dot(hitFromPlayer) <= 0.1) {
                    if (onMiss) onMiss();
    ...
    GameHitscan.fire = function (camera, onHit, onMiss) {
        ...
        return weapon.singleHitFromPellets
    ...
    GameHitscan.getReticleSpec = function (weaponId) {
        ...
        if (weapon.singleHitFromPellets || id === 'shotgun') {
            return {
                type: 'circle',
                size: getCircleReticleSizePx(weapon),
                spreadRadiusPx: getWeaponSpreadRadiusPx(weapon),
                bloomSize: 0,
