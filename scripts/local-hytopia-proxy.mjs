// Local dev proxy: browsers reject the Hytopia dev server's self-signed
...
function createProxy({ name, listenPort, targetPort }) {
  ...
  server.listen(listenPort, LISTEN_HOST, () => {
    console.log(`Local Hytopia proxy (${name}): http/ws://${LISTEN_HOST}:${listenPort} -> https/wss://${TARGET_HOST}:${targetPort}`);
  });
